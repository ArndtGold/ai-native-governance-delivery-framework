...
| 2 | `claude-evaluator-tool-enforcement-implementation` | Implement `evaluators/claude.js` using headless `-p` invocation with `--disallowedTools "Edit,Write,Bash"` plus the existing git-diff mutation check, upgrading Claude's enforcement level from `instruction_only` to `tool_enforced` | Needs UR |  | [Context Graph](CONTEXT_GRAPH.md#CG-DELIVERY-PATH-SEARCH) (claude_enforcement_finding) | Formulate a UR when prioritized; verified technical path exists, not yet implemented |
