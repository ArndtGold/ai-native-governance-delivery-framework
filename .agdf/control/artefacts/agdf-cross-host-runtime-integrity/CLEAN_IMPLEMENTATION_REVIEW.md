...
- evidence: `local-marketplace.js` owns all three classifications and the transaction; `plugin-installers.js` only projects the resulting evidence; the release workflow owns the separate `latest` dist-tag readiness check while the public bootstrap executes the already verified exact release version; focused, public-bootstrap and full regressions pass.
...
- workaround_or_shim_risk: low. No cache patch, registry fallback, copied historical plugin content, platform skip or weakened negative assertion was introduced. Exact-version bootstrap removes a registry-cache race rather than retrying or masking it.
