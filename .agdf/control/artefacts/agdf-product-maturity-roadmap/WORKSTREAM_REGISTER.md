...
| WS-04 | Unified Journey | Progressive Disclosure über stabile Target-/Routing-/Enforcement-Zustände | WS-01 bis WS-03 liefern stabile Semantik | Interaction Contract und `interaction-presentation.js` | none | `evidence_obligation_ready` | All three canonical journey owners are accepted and no separate product gap is evidenced; [Understandability UAT Protocol](UNDERSTANDABILITY_UAT_PROTOCOL.md) is ready for eight direct blinded observations |
| WS-05 | Roadmap Aggregation | PMR-1 bis PMR-7 aus Child-Evidenz bewerten | erforderliche Child-Runs QA/UAT/OR-reif | Roadmap-Run | `agdf-product-maturity-roadmap` | `in_progress` | [Aggregate Acceptance](AGGREGATE_ACCEPTANCE.md): Revision 3 remains startable but not final; PMR-5 is satisfied with limits and PMR-6 has a ready protocol but 0/8 direct observations |
