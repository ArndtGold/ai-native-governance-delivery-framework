...
| WS-04 | Unified Journey | Progressive Disclosure über stabile Target-/Routing-/Enforcement-Zustände | WS-01 bis WS-03 liefern stabile Semantik | Interaction Contract und `interaction-presentation.js` | none | `no_safe_child_scope_yet` | Task Target and Interaction Ownership are accepted; no separate Journey gap is evidenced while Single Install awaits UAT and OpenCode parity remains evidence-limited |
| WS-05 | Roadmap Aggregation | PMR-1 bis PMR-7 aus Child-Evidenz bewerten | erforderliche Child-Runs QA/UAT/OR-reif | Roadmap-Run | `agdf-product-maturity-roadmap` | `in_progress` | [Aggregate Acceptance](AGGREGATE_ACCEPTANCE.md): Revision 2 remains startable but not final; PMR-5 is partial pending Single Install UAT and PMR-6 still lacks direct understandability UAT |
