...
- ui_ux_impact: `medium`
- ui_ux_impact_reason: The change adds visible degradation, version-divergence, automatic install-time SDK alignment and evaluator-capability states and changes both install recovery and the path between executable and instruction-only Delivery Path Search modes.
...
|---|---|---|---|
| OpenCode install and status | partially_done | `create-agdf/lib/installers/opencode.js` already owns the npm invocation, installed package resolution, host/SDK probe, version state, global native surface, session signals and repository activation. | extend |
...
- The OpenCode config installation now resolves `@opencode-ai/plugin` version `1.18.3`, matching the
  installed host after the explicit manual repair that motivated this follow-up.
- The exact registry version `@opencode-ai/plugin@1.18.3` is resolvable, and npm already maintains
  the OpenCode config directory's package manifest and lockfile.
...
3. Keep host, plugin-SDK and AGDF package versions as separate facts. Extend only the existing
   install owner to attempt an exact host-version SDK alignment, verify the observed result and
   retain warning-only status when alignment cannot be completed. Keep `opencode-status` read-only.
...
- Install-time SDK alignment intentionally changes the existing exact SDK dependency and therefore
  needs an explicit package-ownership boundary, exact-version targeting, post-install verification
  and regression coverage for unavailable and failed alignment.
- Alignment must not update the OpenCode host, alter unrelated dependencies, introduce a prompt or
  turn `opencode-status` into a mutating command.
...
2. Which typed install outcomes distinguish aligned, already matching, unavailable, failed and
   uninspectable SDK alignment while preserving warning-only status semantics?
...
- decision: `pass`
- missing_evidence: Product semantics for install-time alignment outcomes, visible degradation,
  preflight proof and fallback behavior remain intentionally owned by the revised PRD/SD.
