...
const validatorCommands = new Set(["target-check", "skill-dispatch", "doctor", "gate-check", "delivery-map", "delivery-path-search", "run-update", "run-approve"]);
...
export async function runValidatorCli(argv = process.argv.slice(2), adapters = {}) {
  ...
  if (parsed.kind === "help") {
    io.log("AGDF surface-local validator: target-check | skill-dispatch | doctor | gate-check | delivery-map | delivery-path-search | run-update | run-approve");
    return 0;
  }
  let options;
  try {
