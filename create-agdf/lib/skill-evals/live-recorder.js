...
function defaultExecute({ surface, cwd, prompt, model, timeoutMs = 120000 }) {
  if (surface === "codex") {
    ...
    try {
      ...
      const args = ["exec", "--sandbox", "read-only", "--ephemeral", "--ignore-user-config", "--skip-git-repo-check", "--output-schema", schemaPath, "--output-last-message", outputPath, "--color", "never"];
      if (model) args.push("--model", model);
      args.push(prompt);
      guardedExecFileSync("codex", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: timeoutMs });
      return readFileSync(outputPath, "utf8");
    } finally { rmSync(temp, { recursive: true, force: true }); }
  }
  if (surface === "claude") {
    const args = ["-p", "--disallowedTools", "Edit,Write,Bash", "--output-format", "json", "--json-schema", JSON.stringify(outputSchema)];
    if (model) args.push("--model", model);
    args.push(prompt);
    const raw = guardedExecFileSync("claude", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: timeoutMs });
