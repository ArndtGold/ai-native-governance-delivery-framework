...
const skills = root => pluginDefinition.skillSet.map(({ slug }) => ({ name: `agdf-${slug}`, source: 'plugin', enabled: true, path: join(root, 'copilot-skills', `agdf-${slug}`) }));
...
function fixtureHost(base) {
  ...
  const exec = (_executable, args) => {
    ...
    if (command === 'skill list --json') return JSON.stringify(state.failSkills ? [] : skills(join(checkout, 'plugins/agdf')));
