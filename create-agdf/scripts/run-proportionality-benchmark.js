...
import { evaluateSeries, loadCorpus, renderMarkdown } from "../lib/proportionality-benchmark/index.js";
...
if (!seriesId || !/^[a-z0-9][a-z0-9._-]{2,79}$/i.test(seriesId)) throw new Error("usage: run-proportionality-benchmark --profile legacy-v1|staged-v2 --series <id> [--json-out path] [--markdown-out path]");
...
for (const testCase of corpus.cases) {
  ...
  const directory = join(root, observationKey);
  try {
