...
function writeOpenCodeInstructions() {
  const lines = [
    ...
    "- AGDF for OpenCode is repo-scoped, not a global OpenCode plugin install. Activation stays local through this repository's `opencode.json`.",
...
function writeOpenCodeReadme(skillSlugs) {
  const lines = [
    ...
    "- AGDF for OpenCode is repo-scoped, not a global OpenCode plugin install; activation stays local through `opencode.json`.",
