...
function writeOpenCodeInstructions() {
  const lines = [
    ...
    "- OpenCode permissions keep `edit` and `bash` on explicit approval and allow the generated `agdf-*` skills explicitly; global adapters use the `agdf-global-*` namespace.",
...
function writeOpenCodeReadme(skillSlugs) {
  const lines = [
    ...
    "- `opencode.json` keeps `edit` and `bash` on `ask` and explicitly allows the generated `agdf-*` skills; global adapters use `agdf-global-*` to avoid same-name masking.",
