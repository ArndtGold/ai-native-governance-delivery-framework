...
if (process.argv.length !== 2) {
  ...
  const surface = process.env.AGDF_SURFACE
    || (process.env.CLAUDE_PLUGIN_ROOT ? "claude" : process.env.PLUGIN_ROOT ? "codex" : "");
  ...
  if (["codex", "claude", "opencode"].includes(surface)) {
    try {
      ...
      if (consentEnabled && ["codex", "claude"].includes(surface)) {
        const pluginRoot = fileURLToPath(new URL("../", import.meta.url));
        const manifest = JSON.parse(readFileSync(new URL("./runtime-manifest.json", import.meta.url), "utf8"));
