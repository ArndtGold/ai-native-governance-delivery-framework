...
Dispatch through the AGDF MCP tool `agdf_dispatch` whenever the host lists it (hosts add a prefix such as `mcp__agdf__agdf_dispatch`; load a deferred tool first): `skill_id` `delivery-closeout`, `presentation_language`, `working_directory`, and only when set the `target_source`/`primary_target` pair and `run_id`.
Otherwise, or when that call fails, use supplied binding schema 2 only: executable, child-only environment and immutable argv_prefix, following binding.arguments exactly with `--skill delivery-closeout`, language and working directory.
