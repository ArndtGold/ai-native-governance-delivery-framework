...
Dispatch through the AGDF MCP tool `agdf_dispatch` whenever the host lists it (hosts add a prefix such as `mcp__agdf__agdf_dispatch`; load a deferred tool first): `skill_id` `delivery-path-search`, `presentation_language`, `working_directory`, and only when set the `target_source`/`primary_target` pair and `run_id`.
